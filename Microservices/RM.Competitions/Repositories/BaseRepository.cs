...
namespace RM.Competitions.Repositories
{
    ...
    {
        public GardensCompetitionContext _context;

        public BaseRepository(GardensCompetitionContext context)
        {
            _context = context;
        }
        ...
        public IEnumerable<T> FindAllByPagination(Expression<Func<T, bool>> criteria, ref ApplicationOperation.Pagination? Pagination, int DefaultPaginationCount, Expression<Func<T, object>> orderBy = null, string orderByDirection = OrderBy.Ascending,
          Expression<Func<T, object>>[] thenByArray = null, string thenByDirection = ThenBy.Ascending, params Expression<Func<T, object>>[] includes)
        {
            ...
            if (Pagination != null)
            {
                ...
                var result = query.AsNoTracking().AsPagination(CurrentPageIndex, DefaultPaginationCount);
                Pagination = new ApplicationOperation.Pagination { RecordPerPage= DefaultPaginationCount, TotalPagesCount = result.TotalPages, CurrentPageIndex = result.CurrentPage, TotalItemsCount = result.TotalItems };
        ...
        public int ExecuteUpdate( Expression<Func<T, bool>> criteria, Func<T, T> Property, Func<T, T> value)
        {
           return _context.Set<T>()
                .Where(criteria)
                .ExecuteUpdate(x=>x.SetProperty(Property,value));
        }

        public async Task<int> ExecuteUpdateAsync(Expression<Func<T, bool>> criteria, Func<T, T> Property, Func<T, T> value)
        {
            return await _context.Set<T>()
                 .Where(criteria)
                 .ExecuteUpdateAsync(x => x.SetProperty(Property, value));
        }
