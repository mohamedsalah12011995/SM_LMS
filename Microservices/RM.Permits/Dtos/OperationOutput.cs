using RM.Core.Helpers;
using static RM.Core.Helpers.Enums;

namespace RM.Permits.Dtos
{
    public class OperationOutput : ApplicationOperation.ResponseOutput
    {
        public class OperationOutputKeys
        {
            public const string PermitsEntity = "PermitsEntity";
            public const string Pagination = "Pagination";
            public const string PermitRequestStatuses = "PermitRequestStatuses";
            public const string GenderList = "GenderList";
            public const string MaritalStatus = "MaritalStatus";
            public const string ActionState = "ActionState";
            public const string WorkSites = "WorkSites";
            public const string Refernces = "Refernces";
            public const string CompanyInfoEntity = "CompanyInfoEntity";
            public const string IdTypeLookups = "IdTypeLookups";
            public const string ProjectEntity = "ProjectEntity";
            public const string FlowStepperEntity = "FlowStepperEntity";
            public const string UsersEntity = "UsersEntity";
            public const string WorkSitesEntity = "WorkSitesEntity";
            public const string EntityID = "EntityID";
            public const string body = "body";
        }
        public static OperationOutput GetOperationOutput(ServiceMessages header, params OutputDictionary[] outputs)
        {
            var result = (OperationOutput)Activator.CreateInstance(typeof(OperationOutput))!;

            if (outputs is not null)
            {
                result.Output = new Dictionary<string, object>();
                foreach (var output in outputs)
                    result.Output.Add(output.key, output.value);
            }
            result.Header = ApplicationOperation.OperationResult(header);


            return result;
        }

    }
    public record OutputDictionary(string key, object value);
}

